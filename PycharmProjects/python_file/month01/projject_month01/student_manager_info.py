...
class StudentManagerController:
    ...
    def score_ascending_order(self):
        """
        根据学生成绩进行升序排列
        :return: 返回排列后的学生列表
        """
        for c in range(len(self.__stu_list)):
            for a in range(len(self.__stu_list)-1):
                if self.__stu_list[a].score>self.__stu_list[a+1].score:
                   self.__stu_list[a], self.__stu_list[a + 1]=self.__stu_list[a+1],self.__stu_list[
                        a]
        return self.__stu_list
...
manager = StudentManagerController()
s01 = StudentModel("zs", 24, 100)
s02 = StudentModel("lisi", 24, 98)
s03 = StudentModel("wangwu", 24, 99)
manager.add_student(s01)
manager.add_student(s02)
manager.add_student(s03)
manager.score_ascending_order()
for item in manager.stu_list:
...
class StudentManagerView:
    ...
    def __input_student(self):
        ...
        if 18 <= age <= 30 or 0>=score>=100:
            stu = StudentModel(name, age, score)
        else:
            raise StudentError("超过我想要的范围啦", age, 26, 1001)
    ...
    def __score_ascending_order(self):
        for item in self.__manager.score_ascending_order():
            print("根据学生成绩的升序排列为："+item.name, item.age, item.score)
